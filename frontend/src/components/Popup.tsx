...
function Popup({ coords, yOffest, imgHeight }: PopupProps) {
  ...
  return (
    <>
      <Draggable
        nodeRef={nodeRef}
        position={{ x: coords.x - 20, y: coords.y - 15 }}
        disabled={true}
      >
        <div
          className={`box absolute bg-red-700 border-black border-2 opacity-50 w-10 h-10`}
          ref={nodeRef}
        ></div>
      </Draggable>
      <Draggable
        nodeRef={nodeRef}
        disabled={true}
        position={{ x: coords.x + 30, y: coords.y - 30 }}
      >
        <div
          className="box absolute flex gap-2 bg-purple-400 p-1.5  hover:cursor-pointer"
          ref={nodeRef}
        >
          {characters.map((char) => (
