    ...
    >
      <div className="content">
        {!found1 && !found2 && (
          <dialog id="dialog" open>
            <div className="fixed bg-blue-900 text-2xl text-white top-1/2 left-1/3 w-fit mx-auto my-0 p-5">
              ...
              <form method="dialog">
                <button className="bg-amber-950  mt-3 p-2 hover:font-bold hover:cursor-pointer">
                  {" "}
                  Got it!
                </button>
              </form>
        ...
        <Draggable
          nodeRef={nodeRef}
          disabled={true}
          position={{ x: contextCoords[0] - 20, y: contextCoords[1] - 15 }}
        >
          <div
            className={`box absolute bg-green-400 border-black border-3 opacity-60 w-10 h-10`}
            hidden={found1}
            ref={nodeRef}
          ></div>
        </Draggable>
        <Draggable
          nodeRef={nodeRef}
          disabled={true}
          position={{ x: contextCoords2[0] - 20, y: contextCoords2[1] - 15 }}
        >
          <div
            className={`box absolute bg-green-400 border-black border-3 opacity-60 w-10 h-10`}
            hidden={found2}
        ...
        {onTarget && (
          <Popup
            yOffest={yOffset}
            imgHeight={yImgClient}
            coords={{ x: xCord, y: yCord }}
          ></Popup>
        )}
